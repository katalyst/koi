  ...
  $(document).on("ornament:tabs", function () {
    ...
    var updateSelect = function ($tabset) {
      ...
      if ($fallbackSelect.length) {
        var activePane = $tabPanes
          .filter(".tabs--pane__active")
          .attr("data-toggle");
        if (activePane) {
    ...
    var checkIfTabsNeedSelectMenu = function ($tabset) {
      ...
      if ($fallbackSelect.length < 1) {
        ...
        $fallbackSelect.on("change", function () {
          ...
          var $toggleAnchor = $tabLinks.find(
            "[data-toggle-anchor=" + tabPaneId + "]"
          );
          $toggleAnchor.click();
          // loadTab($tabset, pane);
    ...
    var loadTab = function (paneId) {
      var $pane = $tabPanes.filter(
        "[data-toggle-tab-pane][data-toggle=" + paneId + "]"
      );
      if ($pane.length) {
