...
(function (document, window, $) {
  ...
  function columnConform() {
    ...
    $conforms.each(function(){
      ...
      $columns.each(function(index) {
        ...
        if( $thisConform.find("[data-conform='"+conformId+"']").length == 2 ) {
          for(currentDiv = 0 ; currentDiv < rowDivs.length ; currentDiv++) setConformingHeightLast(rowDivs[currentDiv], rowDivs, currentTallest);
        } else {
  ...
  $(document).on("ornament:column-conform", function () {
    ...
    $("[data-conform]").find("img").on("load",function(){
      debounceConform();
    });
