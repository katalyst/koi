...
export default class KeyboardController extends Controller {
  ...
  event(cause) {
    ...
    const event = new CustomEvent(action, { detail: { cause: cause }, bubbles: true });
   ...
   */
  describeEvent(event) {
    return [event.ctrlKey && "C", event.metaKey && "M", event.altKey && "A", event.shiftKey && "S", event.code]
      .filter(w => w)
      .join("-");
  }
   ...
   */
  get mappings() {
    const inputs = this.mappingValue.replaceAll(/\s+/g, " ")
      .split(" ")
      .filter(f => f.length > 0);
    const mappings = {};
    ...
    // memoize the result
    Object.defineProperty(this, "mappings", { value: mappings, writable: false });
  ...
  #parse(mappings, mapping) {
    ...
    mappings = keys.reduceRight((mappings, key) => (mappings[key] ||= {}), mappings);
