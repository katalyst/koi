...
RSpec.describe Koi::NavigationLinksController, type: :request do
  ...
  describe "POST /admin/navigations/:slug/links/" do
    let(:action) do
      post koi_engine.navigation_menu_links_path(navigation_menu),
           params: { navigation_link: link_params },
           as:     :turbo_stream
    end
    let(:link_params) { attributes_for(:navigation_link) }
  ...
  describe "PATCH /admin/navigations/:slug/links/:id" do
    ...
    let!(:navigation_link) { create :navigation_link, navigation_menu: navigation_menu, title: title }
    ...
    it "sets title in the new link version" do
      action
      expect(NavigationLink.last.title).to eq("A new level")
    end
