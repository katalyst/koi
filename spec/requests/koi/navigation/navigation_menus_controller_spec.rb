...
RSpec.describe Koi::NavigationMenusController, type: :request do
  ...
  shared_context "with draft" do
    before do
      navigation_menu.update!(items_attributes: [{ id: 1, depth: 0, index: 0 }])
    end
  end
  ...
  describe "POST /admin/navigation_menus" do
    let(:action) { post koi_engine.navigation_menus_path, params: {menu: menu_params} }
  ...
  describe "PATCH /admin/navigations/:slug" do
    ...
    let(:params) { { menu: menu_params, commit: commit } }
    let(:commit) { "discard" }
